...
public class ProveedorDao {
   Conexion cn = new Conexion();
    ...
    // Metodo para registrar Proveedores
     public boolean registrar(Proveedor prov) {
       String sql = "INSERT INTO Proveedor (rfc, nombre, telefono, direccion) VALUES (?,?,?,?)";
       try {
           con = cn.getConexion();
    ...
    public List ListaProveedor(String valor) {
        ...
        String buscar = "SELECT * FROM Proveedor WHERE rfc LIKE '%" + valor + "%' OR nombre LIKE '%" + valor + "%'";
        try {
            con = cn.getConexion();
   ...
   public boolean modificar(Proveedor prov) {
       String sql = "UPDATE Proveedor SET rfc = ?, nombre = ?, telefono =?, direccion = ? WHERE id = ?";
       try {
           con = cn.getConexion();
           ps = con.prepareStatement(sql);
           ps.setString(1, prov.getRfc());
